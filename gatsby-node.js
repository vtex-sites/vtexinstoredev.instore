...
// This removes the _redirects file since all of our redirects are written in
// netlify.toml
exports.onPostBuild = () => {
  const fs = require('fs')

  fs.unlinkSync(`${process.cwd()}/public/_redirects`)

  const nginxConf = `${process.cwd()}/public/nginx.conf`

  const content = fs
    .readFileSync(nginxConf)
    .toString('utf-8')
    .replace(/x-forwarded-host \$origin_host/g, 'x-forwarded-host \'www.marinbrasil.com.br\'')
...
exports.onCreateNode = async ({ node, reporter }) => {
  ...
  reporter.info(`[marinbrasil.store]: Optimizing Images for ${node.name}`)
